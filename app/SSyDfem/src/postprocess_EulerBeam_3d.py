...
#def postprocess_EulerBeam_3d(prop, dimen ,coordinates, elements, u, reaction, Strnod, file_name):
def postprocess_EulerBeam_3d( dimen ,coordinates, elements, u, reaction, file_name):
    """
    ...
    for i in range(npnod):
        ...
        du_xyz.append([ u[i * gl, 0] , u[i * gl+1, 0], u[i * gl+2, 0] ])
        u_xyz.append([ x[i] + u[i * gl , 0] , y[i] + u[i * gl+1, 0] , z[i] + u[i * gl+2, 0] ])
        theta_xyz.append([ u[i * gl , 0], u[i * gl + 1, 0] , u[i * gl + 2, 0] ])
    ...
    m_xyz = []
    for i in range(npnod):
        f_xyz.append( [ reaction[i * gl, 0],reaction[i * gl+1, 0], reaction[i * gl + 2, 0] ] )
        
        m_xyz.append( [ reaction[i * gl+0, 0], reaction[i * gl+1, 0] , reaction[i * gl+2, 0] ] )
